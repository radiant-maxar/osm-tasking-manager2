...
osmtm.project = (function() {
    ...
    switch (editor) {
      ...
      case "iDeditor":
        ...
        if (/^[0-9A-F]{8}-[0-9A-F]{4}-4[0-9A-F]{3}-[89AB][0-9A-F]{3}-[0-9A-F]{12}$/i.test(attribution_config_id)) {
          var presets = `${map_rules_api_url}/${attribution_config_id}/presets/iD`;
          var validations = `${map_rules_api_url}/${attribution_config_id}/rules/iD`;
          url += '&presets=' + presets;
  ...
  return {
    init: function() {
      ...
      checkForUpdates();
      $("#viewMapRulesFrame").attr("src", `${map_rules_url}/${$('#attribution_config_id').val()}/instructions?nav=hide`);
